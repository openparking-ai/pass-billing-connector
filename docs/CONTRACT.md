...
That is 17 findings. A divergence names its side: `billing_only` -- billing holds a row at this garage in this form; no live identity whose stored form the run learned stores in it there; `pass_only` -- the pass has a live identity storing in this form at this garage; billing holds no such row.
<!-- END:findings -->
